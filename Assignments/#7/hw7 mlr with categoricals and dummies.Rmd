...
S = Sweetness M = Moisture

$\hat{Y}= 42.90 + 15.75S + 4.925M - S*M$
...
$M$ = 4.925: The change in mean Rating for sweetness level 2 for a 1 unit increase in Moisture content is 4.925.

$M$ = 3.925: The change in mean Rating for sweetness level 2 for a 1 unit increase in Moisture content is 4.925.
...
xyplot(Price ~ Corner_lot + Valuation, groups = Corner_lot, data = value_data,
       pch = c(1,3), col=c("red","blue"), type = "p",
       main = "Plot of Two Selling Price Lines", key = plotlegend)
```
...
price1 = Coef[1] + Coef[2] * Valuation

#corner lots: When Corner lots takes value 1
price2 = Coef[1] + Coef[2] * Valuation + Coef[3] +Coef[4] * Valuation
...
so the interaction terms [1] 0.00828 is significant and we would keep them in our model.
